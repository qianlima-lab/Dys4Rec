...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def __init__(self, dataModel, config):
        ...
        self.input_seq_action_labels = tf.placeholder(tf.float32, [None, self.seq_length, self.action_nums])

        self.test_input_seq = tf.placeholder(tf.int32, [None, self.seq_length])
        self.test_input_seq_action = tf.placeholder(tf.float32, [None, self.seq_length, self.action_nums])

        self.target_seq_pos = tf.placeholder(tf.int32, [None, self.seq_length])
        self.target_seq_action = tf.placeholder(tf.float32, [None, self.seq_length, self.action_nums])
    ...
    '''
    Action_Fusion_Block: fusion the action of watching a video, 
    input: item embedding [batch size, seq_length, embedding_dims]
    action labels [batch size, seq_length, action_nums]
    output: fusion embedding based action weight 
    '''
    def Action_Fusion_Block(self, item_embedding, action_labels):

        OUTPUT_SIZE = self.numFactor
        # [batch size*seq_length, embedding_dims]
        item_embedding = tf.reshape(item_embedding, shape=[-1, self.numFactor])
        ...
        finished_predict = tf.nn.sigmoid(tf.matmul(finished_embedding, prediction_layer))
        islike_predict = tf.nn.sigmoid(tf.matmul(islike_embedding, prediction_layer))
        iscom_predict = tf.nn.sigmoid(tf.matmul(iscom_embedding, prediction_layer))
        isfollow_predict = tf.nn.sigmoid(tf.matmul(isfollow_embedding, prediction_layer))
        ...
        total_embedding = tf.stack([islike_embedding, iscom_embedding,
                                    isfollow_embedding, finished_embedding], axis=1)
        # [batch size*seq_length, embedding_dims, 1]
        # fusion_embedding = tf.matmul(total_embedding, soft_prediction)
        fusion_embedding = tf.reduce_sum(tf.multiply(total_embedding, soft_prediction), axis=1)
        ...
        return fusion_embedding, action_mse_loss, soft_prediction

    def action_fusion_block(self, item_embedding, action_labels):
        return tf.concat([item_embedding, action_labels], axis=2)

    def action_fusion_embedding_block(self, item_embedding, action_labels):
        # each_action = tf.unstack(action_labels, axis=2)
        # islike, iscom, isfollow, isfinish_play = each_action[:, :, 0], each_action[:, :, 1], each_action[:, :,
        #                                                                                     2], each_action[:, :, 3]
        # islike_onehot, iscom_onehot, isfollow_onehot, isfinish_play_onehot = tf.one_hot(islike, depth=2), tf.one_hot(iscom, depth=2), \
        #                                                                      tf.one_hot(isfollow, depth=2), tf.one_hot(isfinish_play, depth=2)
        action_labels = tf.reshape(action_labels, shape=[-1, self.action_nums])
        action_embedding_dims = 32
        action_embedding_layer = tf.get_variable(
            name="action_embedding_layer",
            dtype=tf.float32,
            shape=[self.action_nums, action_embedding_dims],
            initializer=tf.contrib.layers.xavier_initializer()
        )
        action_embedding_bias = tf.get_variable(
            name="action_embedding_bias",
            dtype=tf.float32,
            shape=[action_embedding_dims],
            initializer=tf.contrib.layers.xavier_initializer()
        )
        action_embedding = tf.matmul(action_labels, action_embedding_layer) + action_embedding_bias
        item_embedding = tf.reshape(item_embedding, shape=[-1, self.numFactor])
        fusion_embedding = tf.concat([item_embedding, action_embedding], axis=1)
        return tf.reshape(fusion_embedding, shape=[-1, self.seq_length, self.numFactor+action_embedding_dims])

    def action_fusion_shared_embedding_block(self, item_embedding, action_labels):
        each_action = tf.unstack(action_labels, axis=2)
        islike, iscom, isfollow, isfinish_play = each_action[:,:,0], each_action[:,:,1], each_action[:,:,2], each_action[:,:,3]
        action_embedding_dims = 8
        shared_layer = tf.get_variable(
            name="fusion_shared_layer",
            dtype=tf.float32,
            shape=[2, action_embedding_dims],
            initializer=tf.contrib.layers.xavier_initializer()
        )
        isfinish_play_embedding = tf.matmul(isfinish_play, shared_layer)
        islike_layer = tf.get_variable(
            name="fusion_shared_layer",
            dtype=tf.float32,
            shape=[2, action_embedding_dims],
            initializer=tf.contrib.layers.xavier_initializer()
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            item_embed_input = tf.reshape(item_embed_input, [-1, self.seq_length, self.numFactor])
            # item_embed_input = tf.nn.dropout(item_embed_input,
            #                                  self.dropout_item)

            # fusion_embedding, action_mse_loss, soft_prediction = self.Action_Fusion_Block(item_embed_input,
            #                                                                               self.input_seq_action_labels)
            # fusion_embedding = self.action_fusion_block(item_embed_input, self.input_seq_action_labels)
            fusion_embedding = self.action_fusion_embedding_block(item_embed_input, self.input_seq_action_labels)
            fusion_embedding = tf.nn.dropout(fusion_embedding,
                                             self.dropout_item)
            ...
            # test_fusion_embedding, _, test_soft_prediction = self.Action_Fusion_Block(test_item_embed_input, self.test_input_seq_action)

            # test_fusion_embedding = self.action_fusion_block(test_item_embed_input, self.test_input_seq_action)
            test_fusion_embedding = self.action_fusion_embedding_block(test_item_embed_input, self.test_input_seq_action)
            rnn_outputs, curr_state = tf.nn.dynamic_rnn(
                cell=self.cell,
                inputs=fusion_embedding,
                dtype=tf.float32,
            ...
            pos_embeds = tf.nn.embedding_lookup(self.itemEmbedding, self.target_seq_pos)
            # pos_embeds = tf.reshape(pos_embeds, [-1, self.seq_length, self.numFactor])
            pos_embeds = tf.reshape(pos_embeds, [-1, 1, self.numFactor])
            # pos_fusion_embedding = self.action_fusion_block(pos_embeds, self.target_seq_action)
            # pos_fusion_embedding = tf.reshape(pos_fusion_embedding, shape=[-1, 1, self.numFactor])
            # pos_fusion_embeds, _, _ = self.Action_Fusion_Block(pos_embeds, self.target_seq_action)
            # pos_fusion_embeds = tf.reshape(pos_fusion_embeds, shape=[-1, 1, self.numFactor])

            # neg_embeds = tf.reshape(tf.nn.embedding_lookup(self.itemEmbedding, self.target_seq_neg),
            #                         [-1, self.neg_num, self.numFactor])
            neg_embeds = tf.nn.embedding_lookup(self.itemEmbedding, self.target_seq_neg)
            # neg_embeds = self.action_fusion_block(neg_embeds,
            #                                       tf.zeros(dtype=tf.float32, shape=[tf.shape(neg_embeds)[0], tf.shape(neg_embeds)[1], self.action_nums]))
            neg_embeds = tf.reshape(neg_embeds, shape=[-1, self.neg_num, self.numFactor])
    ...
    def trainEachBatch(self, epochId, batchId):
        ...
        input_seq_batch, pos_seq_batch, neg_seq_batch, input_seq_action_batch, pos_seq_action_batch = self.getTrainData(batchId)
        # print(batchId)
        self.optimizer.run(feed_dict={
            self.input_seq: input_seq_batch,
            self.target_seq_pos: pos_seq_batch,
            self.target_seq_neg: neg_seq_batch,
            self.input_seq_action_labels: input_seq_action_batch,
            self.target_seq_action: pos_seq_action_batch,
    ...
    def getTrainData(self, batchId):
        ...
        input_seq_action_batch = self.train_sequences_actions_input[start_idx:end_idx]
        pos_seq_batch = self.train_sequences_target[start_idx:end_idx]
        pos_seq_action_batch = self.train_sequences_actions_target[start_idx:end_idx]
        ...
        input_seq_action_batch = np.array(input_seq_action_batch)
        end = time.time()
        # self.logger.info("time of collect a batch of data: " + str((end - start)) + " seconds")
        # self.logger.info("batch Id: " + str(batchId))

        return input_seq_batch, pos_seq_batch, neg_seq_batch, input_seq_action_batch, pos_seq_action_batch
    ...
    def getPredList_ByUserIdxList(self, user_idices):
        ...
        for userIdx in user_idices:
            ...
            input_seq_actions.append(self.user_pred_sequences_actions[userIdx])
        ...
        predList = self.sess.run(self.r_pred, feed_dict={
            ...
            self.test_input_seq_action:input_seq_actions
        })
        end2 = time.time()
