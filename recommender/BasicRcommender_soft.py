...
class BasicRecommender_soft:
    ...
    def __init__(self, dataModel, config):
        ...
        self.bestHit = 0
        self.bestHitBatchId = 0
        self.bestHitEpochId = 0
    ...
    def trainModel(self):
        ...
        for epochId in range(self.maxIter):
            ...
            for batchId in range(self.trainBatchNum):
                ...
                loss, skip_states = self.trainEachBatch(epochId, batchId)
                if skip_states is not None:
                    skip = True
                    totalLoss += loss
                    total_skip_states.append(skip_states)

            if skip:
                total_updated_states_val = np.concatenate(total_skip_states, axis=0)
                print(Counter(total_updated_states_val.reshape(-1)))
    ...
    def evaluateRanking(self, epochId, batchId):
        ...
        #newHit, _ = self.evalRanking.calHit()
        ...
        #self.logger.info("newHit: " + str([newHit]))
    ...
    def saveBestResult(self, newNDCG, newAUC, newPrecision, newRecall, new_t_0, new_t_1, new_t_2, new_t_3,
        ...
        '''
        if new_Hit > self.bestHit:
            self.bestHit = new_Hit
            self.bestHitEpochId = epochId
            self.bestHitBatchId = batchId
        '''
    ...
    def showBestRankingResult(self):
        ...
        self.logger.info("best Hit result: %.4f, batchId: %d, epochId: %d" %
                         (self.bestHit, self.bestHitBatchId, self.bestHitEpochId))
