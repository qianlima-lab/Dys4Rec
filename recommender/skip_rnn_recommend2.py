...
from rnn_cells.supercell_new import LSTMCell
...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def create_rnn_cell_init_state(self, learn_initial_state=True):
        ...
        if isinstance(self.rnn_unit_num_list, list) and len(self.rnn_unit_num_list) > 1:
            if self.cell_type == 'skip_lstm':
                cells = LSTMCell(self.rnn_unit_num_list)
            ...
            if self.cell_type == 'skip_lstm':
                cell = LSTMCell(self.rnn_unit_num)
            elif self.cell_type == 'skip_gru':
                cell = LSTMCell(self.rnn_unit_num)
    ...
    def split_rnn_outputs(self, rnn_outputs):
        ...
        if self.using_skip_rnn():
            return rnn_outputs.h, rnn_outputs.c
        else:
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            rnn_outputs, train_actions = self.split_rnn_outputs(rnn_outputs)
            ...
            test_gru_vector = tf.concat([test_gru_vector, tf.reshape(userEmbedding_test, [-1, self.numFactor])], axis=1)
            ...
            budget_loss = self.compute_budget_loss(train_actions, self.cost)
            ...
            self.skip_states = train_actions
