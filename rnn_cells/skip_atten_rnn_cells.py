...
class SkipLSTMCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, num_units, forget_bias=1.0, activation=tf.tanh, layer_norm=False, update_bias=1.0):
        ...
        self.each_stamp_attention_weight = []
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            num_activity = 4
            multi_h_W = tf.get_variable(
                name="multi_h_W",
                dtype=tf.float32,
                shape=[self._num_units, num_activity * self._num_units],
                initializer=tf.contrib.layers.xavier_initializer()
            )
            multi_h_b = tf.get_variable(
                name="multi_h_b",
                dtype=tf.float32,
                shape=[num_activity * self._num_units],
                initializer=tf.random_uniform_initializer(minval=-0.04, maxval=0.04)
            )
            multi_new_h = tf.matmul(new_h, multi_h_W) + multi_h_b
            # multi_new_h: [batch, num_act, hidden_size]
            multi_new_h = tf.reshape(multi_new_h, shape=[-1, num_activity, self._num_units])
            # transpose_multi_new_h: [num_act, hidden_size, batch]
            transpose_multi_new_h = tf.transpose(multi_new_h, [1, 2, 0])

            mapping_matrix = tf.get_variable(
                name="mapping_matrix",
                dtype=tf.float32,
                shape=[num_activity, 1, self._num_units],
                initializer=tf.contrib.layers.xavier_initializer()
            )
            # shape: [num_act, 1, batch_size]
            attention_weight = tf.matmul(mapping_matrix, transpose_multi_new_h)
            attention_weight = tf.sigmoid(attention_weight)
            # # shape: [1, batch_size, num_act]
            attention_weight = tf.transpose(attention_weight, [2, 1, 0])
            self.each_stamp_attention_weight.append(attention_weight)

            # including attention weight to fusion the hidden_state
            softmax_attention_weight = tf.nn.softmax(attention_weight, axis=2)
            fusion_h = tf.matmul(softmax_attention_weight, multi_new_h)
            fusion_h = tf.reshape(fusion_h, shape=[-1, self._num_units])
            # new_state = SkipGRUStateTuple(new_h, new_update_prob, new_cum_update_prob)
            # new_output = SkipGRUOutputTuple(new_h, update_gate)

            new_state = SkipLSTMStateTuple(new_c, fusion_h, new_update_prob, new_cum_update_prob)
            new_output = SkipLSTMOutputTuple(fusion_h, update_gate)
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            output_fc_b = tf.get_variable(
                name="output_fc_b",
                dtype=tf.float32,
                initializer=tf.constant(0.1, shape=[num_activity * self._num_units])
            )
            multi_new_h = tf.matmul(new_h, output_fc_W) + output_fc_b
            multi_new_h = tf.reshape(multi_new_h, shape=[-1, self._num_units, num_activity])

            mapping_matrix = tf.get_variable(
                name="mapping_matrix",
                dtype=tf.float32,
                shape=[num_activity, 1],
                initializer=tf.contrib.layers.xavier_initializer()
            )
            attention_weight = tf.matmul(multi_new_h, mapping_matrix)
            attention_weight = tf.sigmoid(attention_weight)
...
class SkipGRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            new_state = SkipGRUStateTuple(fusion_h, new_update_prob, new_cum_update_prob)
            new_output = SkipGRUOutputTuple(fusion_h, update_gate)
...
class MultiSkipGRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            new_states.append(SkipGRUStateTuple(multi_new_h, new_update_prob, new_cum_update_prob))
            new_output = SkipGRUOutputTuple(multi_new_h, update_gate)
