...
class DynamicLSTM4RecRecommender(BasicRecommender_soft):
    def __init__(self, dataModel, config):
        ...
        self.cell_gru = tf.contrib.rnn.GRUCell(self.rnn_unit_num)
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            item_embed_input = tf.nn.dropout(tf.reshape(item_embed_input, [-1, self.seq_length, self.numFactor]), self.dropout_item_placeholder)
            ...
            prior_weight = tf.reshape(tf.matmul(split_outputs, self.prior_weight, transpose_b=True), [-1, self.numK, 1])
            context_vector = tf.reshape(
                tf.tanh(tf.matmul(split_outputs, self.output_fc_W, transpose_b=True) + self.output_fc_b),
                [-1, self.numK, self.numFactor])
            context_drop = tf.nn.dropout(context_vector, self.dropout_c2_placeholder)
            ...
            test_prior_weight = tf.reshape(tf.matmul(test_gru_vector, self.prior_weight, transpose_b=True),
                                           [-1, self.numK, 1])
            test_context_vector = tf.reshape(
                tf.tanh(tf.matmul(test_gru_vector, self.output_fc_W, transpose_b=True) + self.output_fc_b),
                [-1, self.numK, self.numFactor])
            ...
            neg_log_prob = tf.reduce_sum(-tf.log(all_act_prob + 0.000001) * tf.one_hot(actions, self.n_actions), axis=2)
    ...
    def test_pred(self, test_prior_weight, test_context_vector, test_item_ids):
        test_item_embedding = tf.reshape(tf.nn.embedding_lookup(self.itemEmbedding, test_item_ids), [-1, self.eval_item_num, self.numFactor])
        if self.numK > 1:
            pred_soft = tf.nn.softmax(tf.reshape(tf.matmul(test_context_vector, test_item_embedding, transpose_b=True), [-1,  self.numK, self.eval_item_num]), axis=2)
            pred_dot = tf.reshape(tf.reduce_sum(tf.multiply(pred_soft, test_prior_weight), axis=1), [-1, self.eval_item_num])
        else:
            pred_dot = tf.reshape(tf.matmul(test_context_vector, test_item_embedding, transpose_b=True), [-1, self.eval_item_num])
        return pred_dot
    ...
    def trainEachBatch(self, epochId, batchId):
        ...
        # print('*' * 20)
        # print([_.eval(feed_dict={
        #     self.input_seq: input_seq_batch,
        # }) for _ in self.RL.actions])
        # print('*' * 20)
        totalLoss += loss
        end = time.time()
