...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def __init__(self, dataModel, config):
        ...
        self.denselayer = tf.get_variable("denselayer", shape=[self.numK * self.numFactor, 2*self.numFactor+self.rnn_unit_num],
                                          initializer=tf.random_uniform_initializer(
                                              minval=-1 / tf.sqrt(float(self.numFactor)),
                                              maxval=1 / tf.sqrt(float(self.numFactor))),
                                          dtype=tf.float32, trainable=True)
    ...
    def buildModel(self):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            #user_padding_embedding = tf.reshape(user_embedding_drop, shape=[-1, self.numFactor])
            ...
            # concat_embedding = tf.concat([tf.reshape(item_embed_input,
            #                                        shape=[-1, self.numFactor]), center_embedding], axis=1)
            ...
            # test_item_embed_input = tf.nn.embedding_lookup(self.itemEmbedding, self.test_input_seq)
            # Batch size x time steps x features.
            # test_item_embed_input = tf.reshape(test_item_embed_input, [-1, 1])

            test_item_embed_input = tf.nn.embedding_lookup(self.itemEmbedding, self.test_input_seq)
            'test seq uid '
            test_item_embed_input = tf.reshape(test_item_embed_input, shape=[-1, self.seq_length, self.numFactor])
            ...
            test_item_embed_input = tf.nn.dropout(tf.reshape(test_fusion_embedding, [-1, self.seq_length, self.numFactor]),
                                             self.drop_memory)
            #test_concat_embedding = tf.concat(
            #    [tf.reshape(test_item_embed_input, shape=[-1, self.numFactor]), test_center_embedding], axis=1)
            #test_fusion_embedding = tf.reshape(
            #    tf.tanh(tf.matmul(test_concat_embedding, self.denselayer, transpose_b=True)
            #            + self.denseBias), [-1, self.numK, self.numFactor])

            test_item_embed_input = tf.nn.dropout(test_item_embed_input, self.drop_memory)
