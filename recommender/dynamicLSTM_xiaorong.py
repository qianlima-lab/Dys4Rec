...
class DynamicLSTM4RecRecommender(BasicRecommender_soft):
    ...
    def __init__(self, dataModel, config):
        ...
        # placeholders
        self.actions = tf.placeholder(tf.int32, [None, 3])
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            user_embedding_new_xiaorong = tf.reshape(gru_vector, [-1, self.numK, self.numFactor])
            ...
            test_user_embedding_new = tf.reshape(tf.tanh(tf.matmul(test_gru_vector, self.denselayer, transpose_b=True)
                                                         + self.denseBias), [-1, self.numK, self.numFactor])
            test_user_embedding_new_xiaorong = tf.reshape(tf.matmul(test_gru_vector, self.denselayer, transpose_b=True)
                                                         + self.denseBias, [-1, self.numK, self.numFactor])
            # test_user_embedding_new_xiaorong = tf.reshape(test_gru_vector, [-1, self.numK, self.numFactor])

            # test_user_embedding_new_xiaorong_arr = test_user_embedding_new_xiaorong.numpy()
            #
            # pca = PCA(n_components=512)

            # reduced_test_user_embedding_new_xiaorong_arr = pca.fit_transform(test_user_embedding_new_xiaorong_arr)
            # reduced_test_user_embedding_new_xiaorong = tf.convert_to_tensor(reduced_test_user_embedding_new_xiaorong_arr, dtype=tf.float32)
            ...
            self.r_pred = self.test_pred(test_prior_weight, test_user_embedding_new_xiaorong, self.pred_seq)
    ...
    def getPredList_ByUserIdxList(self, user_idices):
        ...
        for userIdx in user_idices:
            ...
            #target_seq.append(list(range(1, self.numItem)))
