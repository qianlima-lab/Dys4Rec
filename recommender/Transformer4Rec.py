...
from recommender.bert_modeling import *
...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def __init__(self, dataModel, config):
        ...
        self.hidden_size = self.numFactor
        ...
        self.denselayer = tf.get_variable("denselayer", shape=[self.numK * self.numFactor, self.numFactor+self.hidden_size],
                                          initializer=tf.random_uniform_initializer(
                                              minval=-1 / tf.sqrt(float(self.numFactor)),
                                              maxval=1 / tf.sqrt(float(self.numFactor))),
                                          dtype=tf.float32, trainable=True)
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            # fusion user embedding and item embedding weighted adding: x+a*y a is the weight
            fusion_embedding = tf.add(item_embed_input, tf.multiply(tf.clip_by_value(self.user_embedding_weight, 0.1, 1.0),
                                                                user_padding_embedding))
            item_embed_input = tf.nn.dropout(tf.reshape(fusion_embedding, [-1, self.seq_length, self.numFactor]),
                                             self.drop_memory)

            input_mask = tf.ones(shape=[self.trainBatchSize, 1, self.seq_length], dtype=tf.int32)
            ...
            # transformer_output: list len: layers, [batch, seq, hidden size]
            transformer_output = transformer_model(
                    input_tensor=item_embed_input,
                    ...
                    intermediate_act_fn=get_activation(transformer_config['hidden_act']),
            ...
            split_outputs = tf.nn.dropout(tf.reshape(sequence_output, [-1, self.hidden_size]), self.dropout_context1)
            gru_vector = tf.nn.dropout(tf.concat([tf.reshape(split_outputs, [-1, self.hidden_size]),
                                                  tf.reshape(user_padding_embedding, [-1, self.numFactor])], axis=1),
                                       self.drop_user)
            gru_vector = tf.reshape(gru_vector, [-1, self.hidden_size + self.numFactor])
            ...
            self.skip_states = None
            final_sequence_output = sequence_output[:, -1, :]
            final_embedding = tf.concat([tf.reshape(final_sequence_output, [-1, self.hidden_size]),
                                         tf.reshape(userEmbedding, [-1, self.numFactor])], axis=1)
            final_embedding = tf.reshape(tf.tanh(tf.matmul(final_embedding, self.denselayer, transpose_b=True)
                                                 + self.denseBias), [-1, self.numK, self.numFactor])

            self.r_pred = self.test_pred(test_prior_weight=None, test_context_vector=final_embedding, test_item_ids=self.pred_seq)
