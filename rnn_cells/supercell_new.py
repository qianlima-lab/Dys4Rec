...
SkipLSTMStateTuple = collections.namedtuple("SkipLSTMStateTuple", ("c", "h", "actions", "act_prob"))
SkipLSTMOutputTuple = collections.namedtuple("SkipLSTMOutputTuple", ("h", "actions"))
...
def orthogonal(shape):
  #shape = float(shape)
  flat_shape = (shape[0], np.prod(int(shape[1])))
...
RL = PolicyGradient(n_actions=5, n_features=200)
...
class LSTMCell(tf.contrib.rnn.RNNCell):
  ...
  def __init__(self, num_units, RL=PolicyGradient(n_actions=5, n_features=200),
               forget_bias=1.0, use_layer_norm=False,
  ...
  def __call__(self, inputs, state, scope=None):
    with tf.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      if cur_time == 0:
        ...
        self.RL.choose_action(tf.concat([inputs,h],1), cur_time)
        # each batch clear the matrix
        ...
        c, h = self.get_newch(state, inputs, cur_time, self.RL) #compute the new states
      ...
      concat = tf.concat([inputs, h], 1) # concat for speed.
      ...
      new_output = SkipLSTMOutputTuple(new_h, self.RL.actions)

    return new_output, tf.contrib.rnn.LSTMStateTuple(new_c, new_h)
  ...
  def trainable_initial_state(self, batch_size):
    ...
    with tf.variable_scope('initial_c'):
      initial_c = rnn_ops.create_initial_state(batch_size, self.num_units)
    with tf.variable_scope('initial_h'):
      initial_h = rnn_ops.create_initial_state(batch_size, self.num_units)
    with tf.variable_scope('initial_actions'):
      initial_actions = rnn_ops.create_initial_state(batch_size, 1, trainable=False,
                                                         initializer=tf.zeros_initializer())
    with tf.variable_scope('initial_act_prob'):
      initial_act_prob = rnn_ops.create_initial_state(batch_size, 1, trainable=False,
                                                             initializer=tf.zeros_initializer())
    return SkipLSTMStateTuple(initial_c, initial_h, initial_actions, initial_act_prob)
...
class HyperLSTMCell(tf.contrib.rnn.RNNCell):
  ...
  def __init__(self, num_units, forget_bias=1.0,
    ...
    self.hyper_cell= LSTMCell(hyper_num_units, use_layer_norm=use_layer_norm,
                              use_recurrent_dropout=hyper_use_recurrent_dropout, dropout_keep_prob=dropout_keep_prob)
