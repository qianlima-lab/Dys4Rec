...
class Modify_SkipGRUCell(tf.nn.rnn_cell.RNNCell):
    """
    Single Skip GRU cell. Augments the basic GRU cell with a binary output that decides whether to
    update or copy the cell state. The binary neuron is optimized using the Straight Through Estimator.
    """
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            with tf.variable_scope('skip_decision'):
                update_prob = rnn_ops.linear([inputs, state], 1, True, bias_start=self._update_bias)
                update_prob = tf.sigmoid(update_prob)

            update_gate = _binary_round(update_prob)

            normal_new_h = u * state + (1 - u) * c
            new_h = update_gate * normal_new_h + (1. - update_gate) * state
            new_output = SkipGRUOutputTuple(new_h, update_gate)

        return new_output, new_h
