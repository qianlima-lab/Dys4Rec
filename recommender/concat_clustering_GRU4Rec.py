...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            concat_embedding = center_embedding
            fusion_embedding = tf.concat([item_embed_input, concat_embedding], 1)

            item_embed_input = tf.nn.dropout(tf.reshape(fusion_embedding, [128, self.seq_length, self.numFactor]),
                                             self.drop_memory)
            ...
            test_concat_embedding = tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
                                                test_center_embedding)
            test_fusion_embedding = tf.concat([test_item_embed_input, test_concat_embedding], 1)

            test_item_embed_input = tf.reshape(fusion_embedding, [128, self.seq_length, self.numFactor])
