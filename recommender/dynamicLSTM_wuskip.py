...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            fusion_embedding = tf.nn.dropout(tf.reshape(fusion_embedding, [-1, self.seq_length, self.numFactor]),
                                             self.dropout_item)
            # test input
            ...
            userEmbedding_test = tf.reshape(tf.nn.embedding_lookup(self.userEmbedding, self.u_id_test),
                                            [-1, self.numFactor])
            userEmbedding_test_padding = tf.tile(userEmbedding_test, [1, self.seq_length])
            userEmbedding_test_padding = tf.reshape(userEmbedding_test_padding, shape=[-1, self.seq_length, self.numFactor])

            test_fusion_input = tf.add(test_item_embed_input,
                                      tf.multiply(tf.clip_by_value(self.user_embedding_weight, 0.1, 1.0),
                                                  userEmbedding_test_padding))
            '''
            clustering_params = {
