...
class BasicDataModel:
        ...
        for line in lines:
            ...
            for elements in record[1:]:
                ...
                if self.contains_time:
                    ...
                    self.wholeDataSet.append([userId, itemId])
    ...
    def split_UserTimeRatio(self):
        ...
        for userId, user_items in self.user_items.items():
            ...
            items = user_items
            for itemId in items[:-2]:
                trainSet.append([userId, itemId])
                for element in itemId:
                    itemsInTrainSet.add(element)
                usersInTrainSet.add(userId)
        ...
        for record in testSet:
            userId, itemId = record
            if userId in usersInTrainSet and element in itemsInTrainSet:
                testSetToWrite.append(
                    str(userId) + " " + str(itemId) + " " + " "
                    + '\n')
            else:
