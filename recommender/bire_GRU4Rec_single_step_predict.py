...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def __init__(self, dataModel, config):
        ...
        self.target_seq_pos = tf.placeholder(tf.int32, [None, 1])
    ...
    def create_rnn_cell_init_state(self, learn_initial_state=True):
        ...
        if isinstance(self.rnn_unit_num, list) and len(self.rnn_unit_num) > 1:
            if self.cell_type == 'skip_lstm':
                cells = MultiSkipLSTMCell(self.rnn_unit_num)
            elif self.cell_type == 'skip_gru':
                cells = MultiSkipGRUCell(self.rnn_unit_num)
            elif self.cell_type == 'lstm':
                cell_list = [BasicLSTMCell(n) for n in self.rnn_unit_num]
                cells = tf.contrib.rnn.MultiRNNCell(cell_list)
            elif self.cell_type == 'gru':
                cell_list = [BasicGRUCell(n) for n in self.rnn_unit_num]
                cells = tf.contrib.rnn.MultiRNNCell(cell_list)
            ...
            if isinstance(self.rnn_unit_num, list):
                self.rnn_unit_num = self.rnn_unit_num[0]
            if self.cell_type == 'skip_lstm':
    ...
    def compute_budget_loss(self, updated_states):
        """
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            reverse_input_seq = tf.reverse(self.input_seq, axis=[1])
            reverse_item_embed_input = tf.nn.embedding_lookup(self.itemEmbedding, reverse_input_seq)
            # Batch size x time steps x features.
            item_embed_input = tf.nn.dropout(tf.reshape(item_embed_input, [-1, self.seq_length, self.numFactor]),
                                             self.dropout_item)
            reverse_item_embed_input = tf.nn.dropout(tf.reshape(reverse_item_embed_input, [-1, self.seq_length, self.numFactor]),
                                             self.dropout_item)
            ...
            budget_loss = self.compute_budget_loss(train_updated_states)
            if self.loss_type == 'bpr':
                self.cost = self.get_bpr_pred(element_pos, element_neg) + self.skip_parameter * budget_loss
            else:
                self.cost = self.get_soft_pred(prior_weight, element_pos, element_neg) + self.skip_parameter * budget_loss
