...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def create_rnn_cell_init_state(self, learn_initial_state=True):
        ...
        if isinstance(self.rnn_unit_num_list, list) and len(self.rnn_unit_num_list) > 1:
            ...
            if self.cell_type == 'skip_lstm':
                ...
                #cell = Modify_SkipGRUCell(self.rnn_unit_num)
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            rnn_outputs, train_update_states = self.split_rnn_outputs(rnn_outputs)
            ...
            test_rnn_outputs, test_update_states = self.split_rnn_outputs(test_rnn_outputs)
            '''
            ...
            test_gru_vector = tf.nn.dropout(tf.concat([tf.reshape(test_split_outputs, [-1, self.rnn_unit_num]),
                                                  tf.reshape(user_padding_embedding_test, [-1, self.numFactor])], axis=1),
                                       self.drop_user)

            #test_gru_vector = tf.concat([test_gru_vector, tf.reshape(user_padding_embedding_test, [-1, self.numFactor])], axis=1)
            ...
            if self.loss_type == 'bpr':
                self.cost = self.get_bpr_pred(element_pos, element_neg) + budget_loss
            else:
                self.cost = self.get_soft_pred(prior_weight, element_pos, element_neg) + budget_loss
            ...
            self.skip_states = train_update_states
