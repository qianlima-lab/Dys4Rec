...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            #center_embedding = tf.tile(center_embedding, [10, 1])
            concat_embedding = tf.concat([tf.reshape(item_embed_input, shape=[-1, self.numFactor]), center_embedding],
                                         1)
            fusion_embedding = tf.matmul(concat_embedding, self.denselayer, transpose_b=True)

            fusion_embedding = tf.reshape(fusion_embedding, [-1, self.numK, self.numFactor])


            # pooling version
            format_embedding = tf.reshape(fusion_embedding, shape=[-1, self.numK, self.numFactor])
            #format_tile_embedding = tf.tile(format_embedding, [1, 10, 1])
            ...
            test_q = self.clustering_layer.soft_assignment(embeddings=test_context_vector,
            ...
            #test_center_embedding = tf.tile(test_center_embedding, [10, 1])
            test_concat_embedding = tf.concat(
                [tf.reshape(test_item_embed_input, shape=[-1, self.numFactor]), test_center_embedding], axis=1)
            test_fusion_embedding = tf.reshape(tf.matmul(test_concat_embedding, self.denselayer, transpose_b=True)
                        , [-1, self.numK, self.numFactor])
            #test_fusion_embedding = tf.tile(test_fusion_embedding, [1, 10, 1])
            ...
            element_pos = tf.matmul(format_embedding, pos_embeds, transpose_b=True)
            element_neg = tf.matmul(format_embedding, neg_embeds, transpose_b=True)
