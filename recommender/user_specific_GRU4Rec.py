...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            gru_vector = tf.nn.dropout(tf.concat([split_outputs, user_padding_embedding, dot_embedding], axis=1),
                                       self.drop_user)
            gru_vector = tf.reshape(gru_vector, [-1, self.rnn_unit_num + self.numFactor + self.numFactor])
            ...
            test_gru_vector = tf.concat([test_gru_vector, userEmbedding_test, test_dot_embedding], axis=1)

            test_gru_vector = tf.reshape(test_gru_vector, [-1, 2*self.rnn_unit_num + self.numFactor])
