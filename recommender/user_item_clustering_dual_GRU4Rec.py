...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            cluster_split_outputs = tf.nn.dropout(tf.reshape(cluster_rnn_outputs, [-1, self.rnn_unit_num]),
                                                  self.dropout_context1)
            ...
            W = tf.get_variable(
                name="fc_W",
                dtype=tf.float32,
                shape=[self.rnn_unit_num*2, self.numFactor],
                initializer=tf.contrib.layers.xavier_initializer()
            )

            fusion_outputs = tf.matmul(tf.concat([split_outputs, cluster_split_outputs], axis=1), W)
            context_vector = tf.tanh(fusion_outputs)
            ...
            # test_fusion_vector = tf.add(test_gru_vector,
            #                             tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
            #                                         test_cluster_gru_vector))

            test_fusion_outputs = tf.matmul(tf.concat([test_gru_vector, test_cluster_gru_vector], axis=1), W)
            test_context_vector = tf.tanh(test_fusion_outputs)
