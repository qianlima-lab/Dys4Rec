...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE) as scope:
            ...
            test_fusion_embedding = tf.add(test_fusion_embedding,
                                           tf.multiply(tf.clip_by_value(self.item_center_embedding_weight, 0.1, 1.0),
            ...
            clustering_params = {
                ...
                'alpha': 1,
                'embedding': tf.reshape(user_embedding_new, shape=[-1, self.numFactor])
            }

            self.clustering_layer = DeepTemporalClustering(params=clustering_params)
            seq_center_embedding = tf.nn.embedding_lookup(self.clustering_layer.mu, self.clustering_layer.pred)
            seq_center_embedding = tf.reshape(seq_center_embedding, shape=[-1, self.numK, self.numFactor])

            prior_weight = tf.reshape(tf.matmul(split_outputs, self.prior_weight, transpose_b=True),
                                      [-1, self.numK, 1])
            context_drop = tf.nn.dropout(seq_center_embedding, self.dropout_context2)
            ...
            if self.loss_type == 'bpr':
                self.cost = self.get_bpr_pred(element_pos, element_neg) + budget_loss + self.clustering_layer.loss_kl
            else:
