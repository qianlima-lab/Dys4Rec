...
from skiprnn.src.rnn_cells.skip_rnn_cells import SkipGRUCell
#from component.RNN import RNN_Compoment
#from component.MLP import MLP


class SkipGRU4RecRecommender(BasicRecommender_soft):
    def __init__(self, dataModel, config):
        super(SkipGRU4RecRecommender, self).__init__(dataModel, config)
        self.name = 'SkipGRU'
        ...
        self.cell = SkipGRUCell(self.rnn_unit_num)
        self.initial_state = self.cell.trainable_initial_state(config['trainBatchSize'])
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            rnn_outputs = rnn_outputs.h
            test_rnn_outputs = test_rnn_outputs.h
