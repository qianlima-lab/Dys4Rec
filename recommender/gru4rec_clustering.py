...
from recommender.clustering_layer import DeepTemporalClustering
...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            prior_weight = tf.reshape(tf.matmul(split_outputs, self.prior_weight, transpose_b=True),
                                      [-1, self.numK, 1])
            context_vector = tf.nn.softmax(tf.matmul(split_outputs, self.output_fc_W, transpose_b=True)
                                                + self.output_fc_b)
            # [batch, embed_dims]
            context_vector = tf.contrib.layers.batch_norm(context_vector)
            context_drop = tf.nn.dropout(context_vector, self.dropout_context2)

            # adding the clustering center
            clustering_params = {
                'n_clusters': 10,
                'alpha': 1,
                'embedding': context_drop
            }

            self.clustering_layer = DeepTemporalClustering(params=clustering_params)
            # weight add the center [batch, embedding_dims]
            # center_fusion_embedding = tf.matmul(self.clustering_layer.p, self.clustering_layer.mu)
            center_embedding = tf.nn.embedding_lookup(self.clustering_layer.mu, self.clustering_layer.pred)
            fusion_embedding = tf.add(context_drop,
                                      tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
                                                  center_embedding))
            # pooling version
            format_embedding = tf.reshape(fusion_embedding, shape=[-1, self.numK, self.numFactor])
            #format_embedding = tf.nn.dropout(tf.reshape(format_embedding, [-1, self.numK, self.numFactor]),
            #                                 self.dropout_context2)
            ...
            test_prior_weight = tf.reshape(tf.matmul(test_gru_vector, self.prior_weight, transpose_b=True),
                                           [-1, self.numK, 1])
            test_context_vector = tf.nn.softmax(tf.matmul(test_gru_vector, self.output_fc_W, transpose_b=True)
                                                     + self.output_fc_b)

            test_q = self.clustering_layer.soft_assignment(embeddings=test_context_vector, cluster_centers=self.clustering_layer.mu)
            ...
            test_fusion_embedding = tf.add(test_context_vector,
                                      tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
                                                  test_center_embedding))
            test_format_embedding = tf.reshape(test_fusion_embedding, shape=[-1, self.numK, self.numFactor])
            ...
            pos_embeds = tf.reshape(pos_embeds, shape=[-1, 100])
            pos_clustering_assignment = self.clustering_layer.soft_assignment(pos_embeds, self.clustering_layer.mu)
            pos_pred = tf.argmax(pos_clustering_assignment, axis=1)
            pos_clustering_embedding = tf.nn.embedding_lookup(self.clustering_layer.mu, pos_pred)
            pos_embedding = tf.add(pos_embeds,
                                      tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
                                                  pos_clustering_embedding))
            # pooling version
            pos_embedding = tf.reshape(pos_embedding, shape=[-1, self.numK, self.numFactor])
            element_pos = tf.matmul(format_embedding, pos_embedding
                                    , transpose_b=True)

            neg_embeds = tf.reshape(neg_embeds, shape=[-1, 100])
            neg_clustering_assignment = self.clustering_layer.soft_assignment(neg_embeds,self.clustering_layer.mu)
            neg_pred = tf.argmax(neg_clustering_assignment, axis=1)
            neg_clustering_embedding = tf.nn.embedding_lookup(self.clustering_layer.mu, neg_pred)
            neg_embedding = tf.add(neg_embeds,
                                   tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
                                               neg_clustering_embedding))
            # pooling version
            neg_embedding = tf.reshape(neg_embedding, shape=[-1, self.neg_num, self.numFactor])

            element_neg = tf.matmul(format_embedding, neg_embedding
                                    , transpose_b=True)
            ...
            self.r_pred = self.test_pred(test_prior_weight, test_format_embedding, self.pred_seq)
    ...
    def trainEachBatch(self, epochId, batchId):
        ...
        if epochId == 0 and batchId == 0:
            # init mu
            z = self.sess.run(self.clustering_layer.z, feed_dict={
                self.input_seq: self.train_sequences_input,
                self.dropout_keep_placeholder: self.dropout_keep
            })
            assign_mu_op = self.clustering_layer.get_assign_cluster_centers_op(z)
