...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            # deep kmeans
            unique_input_id, index = tf.unique(tf.reshape(self.input_seq, shape=[-1]))
            unique_input_id = tf.reshape(unique_input_id, shape=[-1, 1])
            part_item_hidden = tf.nn.embedding_lookup(self.itemEmbedding, unique_input_id)
            part_item_hidden = tf.reshape(part_item_hidden, shape=[-1, self.numFactor])

            batch_F = tf.nn.embedding_lookup(self.F, unique_input_id)
            batch_F = tf.reshape(batch_F, shape=[-1, self.K])

            # W has shape [hidden_size, batch_size*seq_length]
            W = tf.transpose(part_item_hidden)
            WTW = tf.matmul(part_item_hidden, W)

            FTWTWF = tf.matmul(tf.matmul(tf.transpose(batch_F), WTW), batch_F)
            ...
            # spectral_embedding = tf.concat([self.item_hidden, batch_F], axis=1)
            # mapping_W = tf.get_variable(
            #     name="mapping_W",
            #     dtype=tf.float32,
            #     shape=[self.numFactor + self.K, self.numFactor],
            #     initializer=tf.contrib.layers.xavier_initializer()
            # )
            #
            # mapping_b = tf.get_variable(
            #     name="mapping_b",
            #     dtype=tf.float32,
            #     initializer=tf.constant(0.1, shape=[self.numFactor])
            # )
            # spectral_embedding = tf.reshape(tf.tanh(tf.matmul(spectral_embedding, mapping_W, transpose_b=False)
            #                                         + mapping_b), [-1, self.seq_length, self.numFactor])
            ...
            # fusion_embedding = tf.nn.dropout(fusion_embedding, self.drop_memory)
