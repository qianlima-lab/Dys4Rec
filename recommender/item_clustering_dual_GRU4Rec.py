...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def __init__(self, dataModel, config):
        ...
        self.center_embedding_weight = tf.Variable(tf.constant(0.5, shape=[1, self.rnn_unit_num]))
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            center_embedding = tf.nn.tanh(tf.matmul(self.clustering_layer.p, self.clustering_layer.mu))
            center_embedding = tf.reshape(center_embedding, shape=[-1, self.seq_length, self.numFactor])

            with tf.variable_scope("item_embedding_rnn"):
            ...
            with tf.variable_scope("center_embedding_rnn"):
                cluster_cell = tf.contrib.rnn.GRUCell(self.rnn_unit_num)
                cluster_rnn_outputs, _ = tf.nn.dynamic_rnn(
                    cell=cluster_cell,
                    inputs=center_embedding,
                    dtype=tf.float32,
                )

            split_outputs = tf.nn.dropout(tf.reshape(rnn_outputs, [-1, self.rnn_unit_num]), self.dropout_context1)
            cluster_split_outputs = tf.nn.dropout(tf.reshape(cluster_rnn_outputs, [-1, self.rnn_unit_num]), self.dropout_context1)

            fusion_outputs = tf.add(split_outputs, tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
                                                                cluster_split_outputs))

            prior_weight = tf.reshape(tf.matmul(fusion_outputs, self.prior_weight, transpose_b=True),
                                      [-1, self.numK, 1])
            context_vector = tf.tanh(tf.matmul(fusion_outputs, self.output_fc_W, transpose_b=True)
                                                + self.output_fc_b)
            ...
            test_center_embedding = tf.nn.tanh(tf.matmul(test_p, self.clustering_layer.mu))
            ...
            test_cluster_rnn_outputs, _ = tf.nn.dynamic_rnn(
                cell=cluster_cell,
                inputs=test_center_embedding,
                dtype=tf.float32,
            )
            ...
            test_cluster_outputs = tf.reshape(test_cluster_rnn_outputs, [-1, self.seq_length, self.rnn_unit_num])
            test_cluster_gru_vector = tf.reshape(test_cluster_outputs[:, -1:, :], [-1, self.rnn_unit_num])

            test_prior_weight = tf.reshape(tf.matmul(test_gru_vector, self.prior_weight, transpose_b=True),
                                           [-1, self.numK, 1])

            test_fusion_vector = tf.add(test_gru_vector, tf.multiply(tf.clip_by_value(self.center_embedding_weight, 0.1, 1.0),
                                                                test_cluster_gru_vector))

            test_context_vector = tf.tanh(tf.matmul(test_fusion_vector, self.output_fc_W, transpose_b=True)
                                                     + self.output_fc_b)
