...
from rnn_cells import rnn_cell_extensions
from rnn_cells import drnn
...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def __init__(self, dataModel, config):
        ...
        self.rnn_unit_num_array = config['rnn_unit_num']
        self.rnn_unit_num = sum(self.rnn_unit_num_array)
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            _, encoder_output_fw = drnn.drnn_layer_final(item_embed_input, self.rnn_unit_num_array, self.dilations, self.seq_length,
                                                         self.numFactor, self.cell_type)

            hidden_state = tf.concat(encoder_output_fw, axis=2)

            split_outputs = tf.nn.dropout(tf.reshape(hidden_state, [-1, self.rnn_unit_num]), self.dropout_context1)
            ...
            test_split_outputs = tf.reshape(hidden_state, [-1, self.seq_length, self.rnn_unit_num])
