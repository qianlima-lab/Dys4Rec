...
class BasicDataModel:
    ...
    def __init__(self, config):
        ...
        self.user_item_train_activity = {}
        self.user_item_test_activity = {}
        ...
        with open(fullPath) as fullFile:
            ...
            for records in record_list:
                ...
                if itemId not in self.itemIdToItemIdx:
                    ...
                    basicItemIdx += 1
                    #file2.write(str(userIdx) + ' ' + str(itemIdx) + '\n')
                else:
                    itemIdx = self.itemIdToItemIdx[itemId]
                    afterPadItemIdx = itemIdx + 1
                    self.itemIdxToItemInfor[itemIdx]['ItemFre'] += 1
                    self.itemIdxToPastUserTimePosition[afterPadItemIdx][userIdx] = len(
                        self.itemIdxToPastUserIdx[afterPadItemIdx])
    ...
    def read_wxg_seq_data(self):
        filepath = './dataset/wxg_seq_data/sessionData1000Users'
        seq_data = pd.read_csv(filepath,
                               names=['fdate', 'fuin', 'feedid', 'mediatype', 'duration', 'time_stamp', 'starttime',
                                      'endtime', 'sessionid', 'session_entertimestamp', 'max_expose_time', 'is_like',
                                      'is_comm', 'is_follow', 'is_finish_play', 'is_feedid_unlike',
                                      'is_finderuin_unlike', 'is_not_follow'
                                      ], sep='\\s+')

        # seq_data = seq_data[(seq_data.is_like == 1) | (seq_data.is_finish_play == 1)
        #                     | (seq_data.is_follow == 1) | (seq_data.is_comm == 1)]

        # filter the user whose seq length is less than 15
        uid_count = seq_data.fuin.value_counts()
        filter_user = uid_count[uid_count > 15].index
        filter_seq_data = seq_data[seq_data.fuin.isin(filter_user)]

        '0 use to represent padding so the number of user and item +1'
        self.numUser = len(filter_user)+1
        self.userSet = set(filter_user)
        self.itemSet = set(np.unique(filter_seq_data.feedid))
        self.numItem = len(self.itemSet)+1

        # format the index
        user_index = [i for i in range(1, self.numUser)]
        self.userIdToUserIdx = dict(zip(filter_user, user_index))
        self.userIdxToUserId = dict(zip(user_index, filter_user))
        self.userIdToUserIdx[0] = 'pad'
        self.userIdxToUserId['pad'] = 0

        item_index = [i for i in range(1, self.numItem)]
        self.itemIdToItemIdx = dict(zip(np.unique(filter_seq_data.feedid), item_index))
        self.itemIdxToItemId = dict(zip(item_index, np.unique(filter_seq_data.feedid)))
        self.userIdToUserIdx[0] = 'pad'
        self.userIdxToUserId['pad'] = 0

        for userId in filter_user:
            userIdx = self.userIdToUserIdx[userId]
            "实际计算时都是采用Idx计算的，而不是采用Id"
            self.user_items_train.setdefault(userIdx, [])
            ...
            target_user_watch_info = filter_seq_data[filter_seq_data.fuin == userId].sort_values(by='time_stamp')
            # transfer to feed index
            target_feed_index = [self.itemIdToItemIdx[feedid] for feedid in target_user_watch_info.feedid.values]
            # activity = ['is_like', 'is_comm', 'is_follow', 'is_finish_play']
            target_user_item_activity = np.array(pd.concat([target_user_watch_info.is_like, target_user_watch_info.is_comm,
                                                          target_user_watch_info.is_follow, target_user_watch_info.is_finish_play], axis=1).values.tolist()).reshape(-1, 4)

            'training_data '
            self.user_items_train[userIdx] = target_feed_index[:-self.training_sequence_length-1]

            for train_item in self.user_items_train[userIdx]:
                self.trainSet.append([userIdx, train_item])

            self.user_item_train_activity[userIdx] = target_user_item_activity[:-self.training_sequence_length-1]
            # build test data
            self.user_items_test[userIdx] = target_feed_index[-self.training_sequence_length-1:]
            self.user_item_test_activity[userIdx] = target_user_item_activity[-self.training_sequence_length-1:]

            for test_item in self.user_items_test[userIdx]:
                self.testSet.append([userIdx, test_item])
    ...
    def read_wxg_seq_data_with_val(self):
        filepath = './dataset/wxg_seq_data/sessionData1000Users'
        seq_data = pd.read_csv(filepath,
                               names=['fdate', 'fuin', 'feedid', 'mediatype', 'duration', 'time_stamp', 'starttime',
                                      'endtime', 'sessionid', 'session_entertimestamp', 'max_expose_time', 'is_like',
        ...
        for userId in filter_user:
            ...
            target_feed_index = [self.itemIdToItemIdx[feedid] for feedid in target_user_watch_info.feedid.values]
            # activity = ['is_like', 'is_comm', 'is_follow', 'is_finish_play']
            target_user_item_activity = np.array(
                pd.concat([target_user_watch_info.is_like, target_user_watch_info.is_comm,
                           target_user_watch_info.is_follow, target_user_watch_info.is_finish_play],
                          axis=1).values.tolist()).reshape(-1, 4)

            'training_data '
            self.user_items_train[userIdx] = target_feed_index[:-2]

            for train_item in self.user_items_train[userIdx]:
                self.trainSet.append([userIdx, train_item])

            self.user_item_train_activity[userIdx] = target_user_item_activity[:-2]

            # build val data
            self.user_items_val[userIdx] = target_feed_index[-2]

            # build test data
            self.user_items_test[userIdx] = [target_feed_index[-1]]
            self.user_item_test_activity[userIdx] = [target_user_item_activity[-1]]
    ...
    def read_tiktop_seq_data_with_val(self):
        filepath = './dataset/tiktok/result.csv'
        seq_data = pd.read_csv(filepath,
                               names=['userid', 'itemid', 'finish', 'like'
                                      ], sep='\\s+')
    ...
    def generate_popular_EvalItemsForEachUser(self):
        "当测试的用户的item数量不足时，随机从不在train set中的item中选出足够的测试item"
        item_popularity = np.zeros(shape=[self.numItem], dtype=int)
        sum_value = 0
        for i in range(1, self.numItem):
            for j in range(1, self.numUser):
                if self.trainMatrix[j][i] != 0:
                    item_popularity[i] += 1
                    sum_value += 1

        probability = [value / sum_value for value in item_popularity]

        for userIdx in self.evalItemsForEachUser:

            "test和train中的itemIdx已经全部加1了"
            itemsToEval = self.user_items_test[userIdx].copy()
            itemsInTrain = self.user_items_train[userIdx]

            while len(itemsToEval) < 101:
                sampled_ids = np.random.choice(self.numItem, 101, replace=False, p=probability)
                sampled_ids = [x for x in sampled_ids if x not in itemsInTrain and x not in itemsToEval]
                itemsToEval.extend(sampled_ids[:])
            item_idx = itemsToEval[:101]
            self.evalItemsForEachUser[userIdx] = item_idx
    ...
    def pre_process(self):
        if self.csv:
            f = csv.reader(open(self.inputPath + '/' + self.splitterType + '/ratings.csv', 'r'))
            next(f)
            for line in f:
            ...
            f = open(self.inputPath + '/' + self.splitterType + '/full1.txt')
            lines = f.readlines()
        ...
        for record in self.wholeDataSet:
            userId, itemId, rating = record
            if userId not in user_items.keys():
                user_items[userId] = []
            user_items[userId].append([itemId, float(rating)])
    ...
    def split_UserTimeRatio(self):
        ...
        for userId, user_items in self.user_items.items():
            # get and sort items
            items = sorted(user_items, key=itemgetter(1))
            for itemId, rating in items[: -2]:
                trainSet.append([userId, itemId, rating])
                itemsInTrainSet.add(itemId)
                usersInTrainSet.add(userId)
            ...
            # items for validSet
            for itemId, rating in items[-2:-1]:
                validSet.append([userId, itemId, rating])
                itemsInTrainSet.add(itemId)
                usersInTrainSet.add(userId)
            ...
            # items for testSet
            for itemId, rating in items[-1:]:
                testSet.append([userId, itemId, rating])

            for itemId, rating in items:
                fullSet.append([userId, itemId, rating])
