...
class GRU4RecRecommender(BasicRecommender_soft):
    ...
    def buildModel(self):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            concat_embedding = tf.concat([tf.reshape(item_embed_input, shape=[-1, self.numFactor]), center_embedding], 1)
            ...
            #                                       center_embedding))
            item_embed_input = tf.nn.dropout(fusion_embedding, self.drop_memory)
